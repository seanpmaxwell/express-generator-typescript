...
function getDepStrings(withAuth, useSocketIo) {
	let dependencies =
		"express dotenv http-status-codes morgan cookie-parser jet-logger " +
		"module-alias command-line-args express-async-errors express-async-handler helmet jsonfile";
	let devDependencies =
		"ts-node typescript nodemon find jasmine supertest " +
	...
	if (withAuth || useSocketIo) {
		...
		devDependencies +=
			" @types/bcrypt @types/randomstring @types/jsonwebtoken";
	}
	if (useSocketIo) {
