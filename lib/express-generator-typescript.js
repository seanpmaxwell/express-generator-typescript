...
async function expressGenTs(destination, useYarn) {
  try {
    ...
    const depStrings = getDepStrings();
...
function getDepStrings() {
  let dependencies = 'express dotenv http-status-codes morgan cookie-parser jet-logger ' +
    'module-alias command-line-args express-async-errors helmet jsonfile bcrypt ' + 
    'jsonwebtoken';
  let devDependencies = 'ts-node typescript nodemon find jasmine supertest ' +
    ...
    'eslint @types/fs-extra @types/bcrypt @types/jsonwebtoken ' + 
    'eslint-plugin-node';
  // Return
