...
const DUMMY_USERS = [
  UserModel.new({ name: 'Sean Maxwell', email: 'sean.maxwell@gmail.com' }),
  UserModel.new({ name: 'John Smith', email: 'john.smith@gmail.com' }),
  UserModel.new({ name: 'Gordan Freeman', email: 'gordan.freeman@gmail.com' }),
] as const;
...
describe('UserRouter', () => {
  ...
  describe(`"GET:${Paths.Users.Get}"`, () => {
    it(
      ...
      async () => {
        const res: IRes<{ users: User[] }> = await agent.get(Paths.Users.Get);
  ...
  describe(`"POST:${Paths.Users.Add}"`, () => {
    it(
      `should return a status code of "${CREATED}" if the request was ` +
        'successful.',
      async () => {
        const user = UserModel.new({ name: 'a', email: 'a@a.com' }),
  ...
  describe(`"PUT:${Paths.Users.Update}"`, () => {
    ...
    it(
      ...
      async () => {
        ...
        const res: IRes = await agent.put(Paths.Users.Update).send({ user });
        expect(res.status).toBe(BAD_REQUEST);
    ...
    it(
      ...
      async () => {
        const user = UserModel.new({ id: 4, name: 'a', email: 'a@a.com' }),
